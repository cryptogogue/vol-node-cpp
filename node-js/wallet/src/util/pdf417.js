...
function getCompaction(mode, code, addmode) {
    ...
    switch(mode) {
        case 900: { // Text Compaction mode latch
            ...
            for (var i = 0; i < codelen; ++i) {
                ...
                var k;
                if ((k = _array_search(chval, TEXTSUBMODULES[submode])) !== false) {
                    ...
                    for (var s = 0; s < 4; ++s) {
                        // search new sub-mode
                        if ((s != submode) && ((k = _array_search(chval, TEXTSUBMODULES[s])) !== false)) {
                            // s is the new submode
                            if ((((i + 1) == codelen) || (((i + 1) < codelen) && (_array_search(_ord(code.charAt(i + 1)), TEXTSUBMODULES[submode]) !== false))) && ((s == 3) || ((s == 0) && (submode == 1)))) {
                                ...
                                // latch
                                txtarr  = txtarr.concat(TEXTLATCH[''+submode+s]);
                                // set new submode
                                submode = s;
                            }
 ...
 *
 */
function _array_fill(start_index, num, mixed_val) {        
    var key, tmp_arr = {};  
...
function _intval(mixed_var, base) {
    // http://kevin.vanzonneveld.net
    // +   original by: Kevin van Zonneveld (http://kevin.vanzonneveld.net)
    // +   improved by: stensi
...
function _preg_split(pattern, subject, limit, flags) {
    // http://kevin.vanzonneveld.net
...
function _array_search(needle, haystack, argStrict) {
  // http://kevin.vanzonneveld.net
  // +   original by: Kevin van Zonneveld (http://kevin.vanzonneveld.net)
  // +      input by: Brett Zamir (http://brett-zamir.me)
  // +   bugfixed by: Kevin van Zonneveld (http://kevin.vanzonneveld.net)
 ...
 */
export const pdf417 = (code, ecl, aspectratio) => {
    ...
    var cols = Math.round((Math.sqrt(4761 + (68 * aspectratio * ROWHEIGHT * nce)) - 69) / 34);
    // adjust cols
    if (cols < 1) {
        cols = 1;
    } else if (cols > 30) {
    ...
    if (QUIETV > 0) {
        empty_row = _array_fill(0, barcode_array['num_cols'], 0);
    ...
    for (var r = 0; r < rows; ++r) {
        ...
        var arow = _preg_split('//', row, -1, 'PREG_SPLIT_NO_EMPTY');
        // duplicate row to get the desired height
        for (var h = 0; h < ROWHEIGHT; ++h) {
            barcode_array['bcode'].push(arow);
        }
        ++cid;
        if (cid > 2) {
            cid = 0;
...
export const barcodeToSVG = ( barcode, bw, bh ) => {
    ...
    // similar. will produce a more efficient mesh *and* get rid of sampling artifacts
    // when SVG is scaled.
    ...
    const extendColumn = ( x, y ) => {

        if ( inColumn && (( columnY + columnHeight ) < y )) {
            flushColumn ();
        }
